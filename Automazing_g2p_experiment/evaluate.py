...
def main(args: argparse.Namespace) -> None:
    ...
    with multiprocessing.Pool() as pool:
        ...
        for (edits, length) in gen:
            ...
            total_edits += edits
            total_length += length
    logging.info(f"{args.tsv_path} WER:\t%.4f", incorrect / (correct + incorrect))
    logging.info(f"{args.tsv_path} LER:\t%.4f", total_edits / total_length)
    #print("f{args.tsv_path} WER:\t%.4f", incorrect / (correct + incorrect))
    #print("f{args.tsv_path} LER:\t%.4f", total_edits / total_length)
    
    
if __name__ == "__main__":
    logging.basicConfig(filename = "log.txt", level="INFO", format="%(levelname)s: %(message)s")
